...
use crate::{bank::sound_asset_bank::SoundAssetBank, source_asset_cache::Checksum, string_hash, tables::{row_locale::RowLocale, row_platform::RowPlatform}};
...
impl ConvertedAssetCache {
    ...
    pub fn get_entry(&self, asset_name: &str, source_checksum: Checksum) -> Option<&ConvertedAssetEntry> {
        self.assets.get(&Self::get_asset_id(string_hash::hash(asset_name), source_checksum))
    }
    ...
    fn load_bank(&mut self, platform: &RowPlatform, language: &RowLocale, file_name: &str) -> Result<(), String> {
    ...
    pub fn add_bank(&mut self, platform: &str, language: &str, bank: SoundAssetBank) -> Result<(), String> {
